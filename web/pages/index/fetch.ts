...
import { orignTrackEnter } from '@/networkApi/track'
...
export default async ({ store, router }: Params, ctx?: ISSRNestContext<{
  ...
  const data = __isBrowser__ ? orignTrackEnter() && await (await window.fetch('/api/index')).json() : await ctx?.apiService?.index()
